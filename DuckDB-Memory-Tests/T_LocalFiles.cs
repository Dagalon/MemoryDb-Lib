...
public class LocalFiles
{
    ...
    [TearDown] public void Cleanup() {  File.Delete(_path); }

    [Test]
    public void GetInstanceCreatesRequestedFile()
    {
        using (var db = DuckTools.GetInstance(_path))
        {
            db.Open();
    ...
    {
        using (var source = new DuckDBConnection(new DuckDBConnectionStringBuilder { DataSource = _path }.ToString()))
        {
            source.Open();
    ...
    {
        using (var db = DuckTools.GetInstance(null))
        {
            db.Open();
            Assert.That(DuckTools.CreateDatabase(db, "local", _path).IsSuccess, Is.True);
            using var cmd = db.CreateCommand();
            cmd.CommandText = "CREATE TABLE local.items(id INTEGER); INSERT INTO local.items VALUES (7)";
            cmd.ExecuteNonQuery();
        }
        using var reopened = DuckTools.GetInstance(null);
        reopened.Open();
        Assert.That(DuckTools.CreateDatabase(reopened, "loaded", _path).IsSuccess, Is.True);
    ...
    public void ResolveSqlReadsFileHeldByWriter()
    {
        ...
        Assert.That(DuckTools.ResolveSql(_path), Is.EqualTo("SELECT 42"));
    }
    ...
    public void ExclusiveLockIsReported()
    {
        ...
        Assert.Throws<IOException>(() => DuckTools.ResolveSql(_path));
    }

    [Test]
    public void ParquetImportReadsFileHeldByWriter()
    {
        using var db = DuckTools.GetInstance(null);
        db.Open();
        using var cmd = db.CreateCommand();
        cmd.CommandText = $"COPY (SELECT 42 AS id) TO '{_path.Replace("'", "''")}' (FORMAT PARQUET)";
        cmd.ExecuteNonQuery();
        using var writer = new FileStream(_path, FileMode.Open, FileAccess.ReadWrite, FileShare.Read);
        Assert.That(QueryExecutor.CreateParquetTable(db, "main", "items", _path), Is.EqualTo(EnumsDuckMemory.Output.SUCCESS));
        cmd.CommandText = "SELECT id FROM items";
        Assert.That(Convert.ToInt32(cmd.ExecuteScalar()), Is.EqualTo(42));
    }
    ...
    public void FailedAttachCanBeRetried()
    {
        ...
        using var db = DuckTools.GetInstance(null);
        db.Open();
        Assert.That(DuckTools.CreateDatabase(db, "retry", _path).IsSuccess, Is.False);
        File.Delete(_path);
        Assert.That(DuckTools.CreateDatabase(db, "retry", _path).IsSuccess, Is.True);
    }
}
